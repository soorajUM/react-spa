...
const Form = <FormData extends Record<string, any>>({
  ...
  const { submit, formData, valid, formError, onChange } = useForm({
    emptyForm,
    defaultValues,
    validateFunction,
    onSubmit,
  ...
  return (
    <form
      onSubmit={submit}
      className="p-10 w-fit shadow-lg rounded-xl flex-col place-items-center flex "
    >
      ...
      <Button label={formSubmitButtonLabel} action="submit" disabled={!valid} />
    </form>
  );
};
